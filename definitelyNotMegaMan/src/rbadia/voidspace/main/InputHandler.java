...
public class InputHandler implements KeyListener{
	...
	private int stack= 0;
	private int mute = 0;
	private boolean trueMute = false;
	...
	public void handleInput(GameScreen gameScreen){
		...
		if(!status.isGameOver() && !status.isNewMegaMan() && !status.isGameStarting() && !status.isGameWon()){
			...
			if (nIsPressed){
				long currentTime = System.currentTimeMillis();
				if((currentTime - lastExchangeTime > 750)){
					lastExchangeTime = currentTime;
					gameLogic.nextKey();
			...
			// MM up mechanic could be cleaner but this one works
			if(upIsPressed){
				long currentTime = System.currentTimeMillis();
				if((currentTime - lastBigBulletTime) > 240){ //if i<10 (700)
					lastBigBulletTime = currentTime;
					for(int i=0; i<6; i++){
			...
			if(downIsPressed){
				long currentTime = System.currentTimeMillis();
				if ((currentTime - lastBigBulletTime) > 100){
					lastBigBulletTime = currentTime;
