...
public class GameLogic {
	...
	private Floor[] floor;
	int[] boomForLevel = {0, 5, 11, 17, 23, 29, 35, 100};
	...
	public void newGame(){
		...
		newNumPlatforms(gameScreen, numPlat);
	...
	public void checkConditions(){
		...
		if (status.getLevel() >= 6 && status.getBossLife()==0){
			status.setShipsLeft(0);
		}
		if(!status.isGameWon()){
			if(gameScreen.getBoom() != 0 && changeLevel())
				gameWon();
		}
	...
	public boolean changeLevel(){
		if (gameScreen.getBoom() == boomForLevel[status.getLevel()])
		{
			return true;
		}
	...
	public void nextKey(){
		...
		gameScreen.setBoom(boomForLevel[status.getLevel()]);
	...
	public Asteroid newAsteroid3(GameScreen screen){
		this.asteroid3 = new Asteroid(screen);
		return asteroid3;
	}
	/**
